...
function mockJson(content: string, meta: Record<string, string> = {}) {
  return {
    ok: true,
    json: async () => ({
      title: meta.title,
      content,
      source: meta.author || meta.site,
    }),
  } as Response;
}
...
describe("improve-ui spec tests", () => {
  ...
  describe("FEEDME-012: 미리보기 영역에 border 없음", () => {
    it("웹페이지 미리보기 영역에 border 관련 클래스가 없다", async () => {
      ...
      global.fetch = vi
        .fn()
        .mockResolvedValue(mockJson("# 테스트\n\n본문 내용"));
  ...
  describe("FEEDME-014: 취소선 렌더링", () => {
    it("~~취소선 텍스트~~가 <del> 요소로 렌더링된다", async () => {
      ...
      global.fetch = vi
        .fn()
        .mockResolvedValue(mockJson("~~취소선 텍스트~~"));
  ...
  describe("FEEDME-016: 태스크리스트 렌더링", () => {
    it("태스크리스트가 체크박스(input[type='checkbox'])로 렌더링된다", async () => {
      ...
      global.fetch = vi
        .fn()
        .mockResolvedValue(mockJson("- [x] 완료\n- [ ] 미완료"));
      ...
      await waitFor(() => {
        ...
        const uncheckedBoxes = container.querySelectorAll(
  ...
  describe("FEEDME-018: 자동 링크 렌더링", () => {
    it("URL 텍스트가 클릭 가능한 <a> 링크로 렌더링된다", async () => {
      ...
      global.fetch = vi
        .fn()
        .mockResolvedValue(
          mockJson("방문하세요 https://example.com 여기에"),
        );
  ...
  describe("FEEDME-019: 언어 미지정 코드 블록 하이라이팅 없음", () => {
    it("언어가 지정되지 않은 코드 블록에 하이라이팅 클래스가 없다", async () => {
      ...
      const languageCode = container.querySelector(
        "code[class*='language-']",
      );
