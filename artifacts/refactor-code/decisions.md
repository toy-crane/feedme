...
## 10. Defuddle API 브라우저 직접 호출 CORS 차단 (2026-04-08)

### 문제
- 브라우저에서 `defuddle.md` 직접 호출 시 CORS 에러 발생
- Defuddle API 코드 자체는 `Access-Control-Allow-Origin: *`를 반환하지만, **Cloudflare Bot Fight Mode**가 Worker 도달 전에 JS challenge를 요구

### 검토 과정
1. **Next.js API Route 프록시 (`/api/extract`)**: 로컬에서는 200 OK, Vercel 배포에서는 403
2. **User-Agent 전달**: 클라이언트 UA를 그대로 전달해도 403 — challenge는 UA 기반이 아님
3. **상세 로깅으로 원인 특정**: Vercel Logs에서 `cf-mitigated: challenge`, body `"Just a moment..."` 확인 → Cloudflare JS challenge (Turnstile)

### 결론
- Defuddle API는 Cloudflare 뒤에 있으며, Bot Fight Mode가 서버(Vercel datacenter) 요청에 JS challenge를 강제
- 서버에서는 JS challenge를 풀 수 없으므로, User-Agent 변경이나 API Route 프록시로는 해결 불가
- 브라우저 직접 호출도 CORS가 막히는 상황이므로, 현재 Defuddle hosted API는 사용 불가

### 해결
- **Defuddle npm 패키지를 Next.js API Route에서 직접 사용** (`defuddle/node` + `linkedom`)
- 외부 API 의존성 완전 제거, 서버에서 대상 URL HTML fetch → defuddle 파싱 → JSON 반환
- YouTube 자막 지원 제거 (datacenter IP 차단으로 어떤 서버에서든 불안정)
- Cloudflare Worker 테스트도 진행했으나 (feedme-extract), YouTube는 동일 이슈 확인
